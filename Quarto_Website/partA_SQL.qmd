...
### SQL Part 1 - Total Budget Allocation for Productions
...
### SQL Part 2 - Average Development Time for Productions
...
### SQL Part 3 - Top Three Crew Members
...
# Links Productions and Clients and selects Client Name, Country, Production Title and Genre
# Finds countries that are tagged UK or Japan and Genres Sci-fi or Drama
# Sorts by Country and production title
...
# Selects Unique crew role listed only once
# Sorts the role from A-Z (Ascending order)
sqlconcept2 <- "SELECT DISTINCT Role FROM CrewMembers ORDER BY ROLE ASC;"

# Queries the database and runs SQL concept demonstration 1
result5 <- dbGetQuery(conn, sqlconcept2)

# Display Table with results
kable(result5, caption = 'Unique Crew Roles in the Studio') %>% kable_styling(full_width = FALSE, position = 'center')

```

### SQL Concept 3 Demonstration
...
# Selects Title and budget from productions and calculates average budget of all productions
# Only productions with above average budget are selected
#Order by descending order
sqlconcept3 <- "SELECT Title, Budget FROM Productions WHERE Budget > (SELECT AVG(Budget) FROM Productions) ORDER BY Budget DESC;"

# Queries the database and runs SQL concept demonstration 1
result6 <- dbGetQuery(conn, sqlconcept3)

# Display Table with results
datatable(result6, caption = 'Productions with the above-average Budgets', options = list(pageLength = 8))
